...
type Cache interface {
	...
	// returned.
	Put(kind EntryKind, projectName string, hash string, size int64, rdr io.Reader) error
	...
	Get(kind EntryKind, projectName string, hash string) (rdr io.ReadCloser, sizeBytes int64, err error)
...
func GetValidatedActionResult(c Cache, projectName string, hash string) (*pb.ActionResult, []byte, error) {
	rdr, sizeBytes, err := c.Get(AC, projectName, hash)
	if err != nil {
		return nil, nil, err
	}
	...
	for _, f := range result.OutputFiles {
		if len(f.Contents) == 0 && f.Digest.SizeBytes > 0 {
			if !c.Contains(CAS, projectName, f.Digest.Hash) {
				return nil, nil, nil // aka "not found"
			}
		}
	}
	...
	for _, d := range result.OutputDirectories {
		if !c.Contains(CAS, projectName, d.TreeDigest.Hash) {
			return nil, nil, nil // aka "not found"
		}
	}

	if result.StdoutDigest != nil {
		if !c.Contains(CAS, projectName, result.StdoutDigest.Hash) {
			return nil, nil, nil // aka "not found"
		}
	}

	if result.StderrDigest != nil {
		if !c.Contains(CAS, projectName, result.StderrDigest.Hash) {
			return nil, nil, nil // aka "not found"
		}
	}
