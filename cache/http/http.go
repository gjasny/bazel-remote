...
func uploadFile(remote *http.Client, baseURL *url.URL, local cache.Cache, accessLogger cache.Logger,
	errorLogger cache.Logger, projectName string, hash string, kind cache.EntryKind) {
	rdr, size, err := local.Get(kind, projectName, hash)
	if err != nil {
		return
	}
	...
	url := requestURL(baseURL, projectName, hash, kind)
	...
	for uploader := 0; uploader < numUploaders; uploader++ {
		go func(remote *http.Client, baseURL *url.URL, local cache.Cache, accessLogger cache.Logger,
			...
			for item := range uploadQueue {
				uploadFile(remote, baseURL, local, accessLogger, errorLogger, item.projectName, item.hash, item.kind)
			}
...
func (r *remoteHTTPProxyCache) Put(kind cache.EntryKind, projectName string, hash string, size int64, rdr io.Reader) error {
	if r.local.Contains(kind, projectName, hash) {
		io.Copy(ioutil.Discard, rdr)
		return nil
	}
	err := r.local.Put(kind, projectName, hash, size, rdr)
	if err != nil {
		return err
	}
...
func (r *remoteHTTPProxyCache) Get(kind cache.EntryKind, projectName string, hash string) (io.ReadCloser, int64, error) {
	if r.local.Contains(kind, projectName, hash) {
		return r.local.Get(kind, projectName, hash)
	}

	url := requestURL(r.baseURL, projectName, hash, kind)
	...
	err = r.local.Put(kind, projectName, hash, sizeBytes, rsp.Body)
	if err != nil {
		return nil, -1, err
	}
...
func (r *remoteHTTPProxyCache) Contains(kind cache.EntryKind, projectName string, hash string) bool {
	return r.local.Contains(kind, projectName, hash)
}
...
func requestURL(baseURL *url.URL, projectName string, hash string, kind cache.EntryKind) string {
	if projectName != "" {
		return fmt.Sprintf("/%s/%s/%s_%s", baseURL, kind, hash, projectName)
	} else {
