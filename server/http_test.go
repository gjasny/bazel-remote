...
func TestDownloadFile(t *testing.T) {
	...
	hash, err := testutils.CreateCacheFile(filepath.Join(cacheDir, "cas"), NO_PROJECT, 1024)
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseRequestURL(t *testing.T) {
	...
	{
		kind, projectName, hash, err := parseRequestURL("cas/"+aSha256sum, true)
		if err != nil {
			t.Error("Failed to parse a valid CAS URL")
		}
		if projectName != "" {
			t.Error("Project name parsed incorrectly")
		}
		if hash != aSha256sum {
	...
	{
		kind, projectName, hash, err := parseRequestURL("ac/"+aSha256sum, true)
		if err != nil {
	...
	{
		kind, projectName, hash, err := parseRequestURL("prefix/ac/"+aSha256sum, true)
		if err != nil {
	...
	{
		kind, projectName, hash, err := parseRequestURL("prefix/ac/"+aSha256sum, false)
		if err != nil {
			t.Error("Failed to parse a valid AC URL with prefix")
		}
		if projectName != "prefix" {
...
func (f *fakeCache) Put(kind cache.EntryKind, projectName string, hash string, size int64, r io.Reader) error {
	return nil
}

func (f *fakeCache) Get(kind cache.EntryKind, projectName string, hash string) (rdr io.ReadCloser, sizeBytes int64, err error) {
...
func (f *fakeCache) Contains(kind cache.EntryKind, projectName string, hash string) (ok bool) {
	return false
}
